...
CREATE TABLE `campañas_usuarios` (
  `id` int NOT NULL AUTO_INCREMENT,
  `usuario_id` int DEFAULT NULL COMMENT 'Identificador de usuario',
  `campaña_id` int DEFAULT NULL COMMENT 'Identificador de campaña',
...
CREATE TABLE `estadisticas_usuario` (
  ...
  `veces_ayudado_compartiendo` int DEFAULT NULL COMMENT 'aqui se mostraran las veces que el usuario mostro apoyo',
  `veces_apoyado_economicamente` int DEFAULT NULL COMMENT 'aqui se mostraran las veces que el usuario mostro apoyo economicamente',
  `dinero_donado` decimal(10,2) DEFAULT NULL COMMENT 'dinero donado por el usuario',
  `nivel_usuario` varchar(50) DEFAULT NULL COMMENT 'aqui se mostrara el nivel que lleva el usuario',
  `proxima_meta` varchar(100) DEFAULT NULL COMMENT 'Proxima menta a lograr por el usuario',
  PRIMARY KEY (`id`),
  KEY `usuario_id` (`usuario_id`),
...
CREATE TABLE `logs` (
  ...
  `accion` varchar(255) DEFAULT NULL COMMENT 'Acción de actualización',
  `fecha` timestamp NULL DEFAULT NULL COMMENT 'Fecha de registro',
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  KEY `usuario_id` (`usuario_id`),
...
CREATE TABLE `puntos_usuarios` (
  `id` int NOT NULL AUTO_INCREMENT,
  `usuario_id` int DEFAULT NULL COMMENT 'Identificador de usuario',
  `campania_id` int DEFAULT NULL COMMENT 'Identificador de campaña',
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `login_usuario`( IN p_nombre_usuario VARCHAR(50),IN p_contraseña VARCHAR (255))
BEGIN
...
-- Dump completed on 2024-08-12 18:42:34
